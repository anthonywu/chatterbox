...
# MPS doesn't support float16/bfloat16 for stft and many complex ops
_DEVICE_REQUIRES_FLOAT32 = frozenset({"mps"})


def _needs_float32_cast(device: torch.device) -> bool:
    """Check if device requires float32 for stft/complex operations."""
    return device.type in _DEVICE_REQUIRES_FLOAT32
...
class S3Tokenizer(S3TokenizerV2):
    ...
    def forward(
        ...
        for wav in processed_wavs:
            if _needs_float32_cast(self.device):
